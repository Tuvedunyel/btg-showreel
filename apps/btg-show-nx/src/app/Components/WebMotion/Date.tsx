...
const Date: FC<{ view: boolean }> = ( { view } ) => {
  ...
  return (
    <motion.div initial={ { opacity: 1 } } whileInView={ { opacity: 0 } } transition={ { duration: 1, delay: 2 } }
                viewport={ { once: true } } className="date__container">
      <motion.div ref={ ref } initial={ { opacity: 0 } } whileInView={ { opacity: 1 } } viewport={ { once: true } }
                  transition={ { duration: 1 } } className="date__hexa__container">
        <Hexagone top={ false } duration={ 0.3 } />
        <Hexagone top={ false } duration={ 0.6 } />
        <Hexagone top={ false } duration={ 0.8 } />
      </motion.div>
      <motion.div initial="init" animate={ animation }
                  variants={ variants }
